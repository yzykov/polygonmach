...
export async function ProductCard({ item }: { item: IndexItem }) {
  const data = await getEntityData("products", item.source_id);
  const image = entityMainImage(data, item.main_image);

  return (
    <Link
      href={`/product/${item.source_id}/`}
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="mt-5 text-sm font-bold text-red-600">Подробнее →</div>
      </div>
    </Link>
  );
}
